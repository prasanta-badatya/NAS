...
| WSGI       | Waitress (Windows-compatible production server)         |
| Static     | WhiteNoise (serves Django admin/static via WSGI)        |
| Frontend   | Angular 16 (TypeScript)                                 |
| Reverse proxy | nginx for Windows — serves Angular, proxies `/api/` |
| Tunnel     | Cloudflare Tunnel — public HTTPS with no port-forward   |
...
**Environment variables** — copy `.env.example` to `.env` (gitignored) and set:

```bash
DJANGO_SECRET_KEY=your-secret-key-here
DJANGO_DEBUG=False
DJANGO_ALLOWED_HOSTS=localhost,<your-tunnel-domain>.trycloudflare.com
CORS_ALLOWED_ORIGINS=https://<your-tunnel-domain>.trycloudflare.com
```
...
## Production deployment — Cloudflare Tunnel (public HTTPS, free)

Access your NAS from anywhere in the world via HTTPS, no port forwarding
required.  Cloudflare handles the certificate; your PC makes an outbound
tunnel connection to Cloudflare's edge.

```
Any browser → https://nas.yourdomain.com (Cloudflare Edge)
                      ↓ encrypted outbound tunnel
            nginx :8080 (Windows)
            ├── /* → nas-ui/dist/nas-ui/ (Angular SPA)
            └── /api/* → Waitress :8000 (Django)
```
...
```bash
cp .env.example .env
# Edit .env — generate a real SECRET_KEY, set your tunnel domain
```
...
Edit `nas-ui/src/environments/environment.prod.ts` — replace
`<your-tunnel-domain>` with your actual Cloudflare tunnel hostname, then:

```bash
cd nas-ui
ng build --configuration production
# Output: nas-ui/dist/nas-ui/
```

### Step 4 — Start the Django backend

```bat
REM Double-click or run in a terminal:
start_server.bat
```

Or manually: `waitress-serve --port=8000 --threads=4 nas_server.wsgi:application`

### Step 5 — Configure nginx

Download nginx for Windows from https://nginx.org/en/download.html, extract to
`C:\nginx\`, then replace `C:\nginx\conf\nginx.conf` with:
...
**Permanent tunnel (free Cloudflare account + domain):**
```bash
cloudflared tunnel login
cloudflared tunnel create nas
cloudflared tunnel route dns nas nas.yourdomain.com
...
1. `http://localhost:8080` → Angular login page
2. Login works → token returned from Django
3. Upload a photo → stored in `NAS_STORAGE/`
4. Cloudflare URL in browser → same app over HTTPS
5. Upload from Cloudflare URL → works with large (500 MB) files
6. Open URL on mobile data (not home Wi-Fi) → confirms global access
